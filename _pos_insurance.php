				...
				<form action="" id="manage-sales">
					...
					<div class="col-md-12">
                        ...
                        <div class="row">
							...
							<div class="col-md-4 form-group hiddens">
							
							</div>
							<!-- <div class="col-md-2" id='qty_available_and_price' style="float: right;">
								<label for="" class="control-label">Available:</label><span class="success" id='available'></span><br>
								<label for="" class="control-label">Cash Price:</label><span class="success" id='price'></span><br>
								<label for="" class="control-label">NHIF Price:</label><span class="success" id='nhif_price'></span><br>
							</div> -->
						...
						<div class="row mb-3 propersales">
								<!-- <div class="col-md-4">
									...
									while($row=$product->fetch_assoc()):
										$prod[$row['id']] = $row;
									?>
										<option value="<?php echo $row['id'] ?>" data-name="<?php echo $row['name'] ?>" data-measurement="<?php echo $row['measurement'] ?>" data-description="<?php echo $row['description'] ?>" data-prescription="<?=$row["prescription"]?>"><?php echo $row['name'] . ' | ' . $type_arr[$row['item_units_id']].' | '.$row['description']. ' | Tsh. ' . $row['price'].' /=' ?></option>
									<?php endwhile; ?>
									</select>
									<small><a href="javascript:void(0)" id="search_prod">Search product in details.</a></small>
								</div> -->
                                <div class="col-md-4 propersales">
									<label class="control-label">Product</label>
					...
					<div class="modal fade" id="prescriber_modal" role='dialog'>
					  ...
					  <!-- <div class="modal fade" id="dept_collector" role='dialog'>
					    <div class="modal-dialog modal-md" role="document">
					      <div class="modal-content">
					        <div class="modal-header">
					        <h5 class="modal-title"></h5>
					      ...
					      <div class="modal-body">
					      	<div class="container-fluid">
					      		<div class="form-group">
					      			...
					      			<input type="text" name="deptor_name"  class="form-control text-right">
					      		</div>
					      		<div class="form-group">
					      			<label for="" class="control-label">PHONE NUMBER</label>
					      			<input type="text" name="deptor_phone_number"  class="form-control text-right" >
					      		</div>
					      		<div class="form-group">
					      			<label for="" class="control-label">DUE DATE</label>
					      			<input type="date" name="dept_end_date"  class="form-control text-right" >
					      		</div>
					      	</div>
					      </div>
					      <div class="modal-footer">
						  ...
						  <div class="container-fluid">
							  <div class="form-group">
									...
									<select name="paymethod" id="paymethod" class="form-control custom-select browser-default select2">
										...
										?>
										<option value="<?php echo $row['payment_method_id'] ?>" data-payment_method="<?php echo $row['payment_method_name']?>" ><?php echo $row['payment_method_name'] ?></option>
										<?php endwhile; ?>
									</select>
					      		</div>
					      		<div class="form-group">
								...
								<div class="dept" id="dept">
								  	<!-- <div class="container-fluid"> -->
										<div class="form-group">
											<label for="" class="control-label">CUSTOMER NAME</label>
											<input type="text" name="deptor_name"  class="form-control ">
										</div>
										<div class="form-group">
											<label for="" class="control-label">PHONE NUMBER</label>
											<input type="tel" name="deptor_phone_number"  class="form-control"  id="deptor_phone_number">
										</div>
										<div class="form-group">
											<label for="" class="control-label">DUE DATE</label>
											<input type="date" name="dept_end_date"  class="form-control" >
										</div>
									<!-- </div> -->
								</div>
								<div class="bank_swip" id="bank_swip">
								  	<!-- <div class="container-fluid"> -->
										<div class="form-group">
											<label for="" class="control-label">TRANSACTION #</label>
											<input type="text" name="bank_transaction"  class="form-control ">
										</div>
									<!-- </div> -->
								</div>
								<div class="lipa_namba" id="lipa_namba">
								  	<!-- <div class="container-fluid"> -->
										<div class="form-group">
											<label for="" class="control-label">NAMBA YA MUAMALA</label>
											<input type="text" name="lipa_namba"  class="form-control ">
										</div>
									<!-- </div> -->
								</div>
					      		<div class="form-group">
...
<script type="text/javascript">
        $(document).ready(function(){
			...
			// $("#deptor_phone_number").usPhoneFormat();
            ...
            $("#scheme").change(function(){
				// $('list tbody #tr_clone tr.item-row').remove();
                var product_id = $(this).val();
				$('#paymethod').attr("disabled",false);
					if (product_id == 1) {
						$('.insurance').hide()
						$('.hiddens').show()
						$('#paymethod option[value="'+2+'"]').attr("disabled",true);
					}if(product_id == 2){
						$('.insurance').show();
						$('#paymethod').val(2).change();
						$('#paymethod').attr("disabled",true);
						$('#dept_collector').modal('show')
						$('.hiddens').hide()
			...
			$("#product").change(function(){
				...
				$.ajax({
					...
					console.log(JSON.stringify(resp))
						...
						$('#qty_available_and_price').find('#available').html(resp.available)
						$('#qty_available_and_price').find('#price').html(resp.price)
						$('#qty_available_and_price').find('#nhif_price').html(resp.price_nhif)
...
<script>
				...
				success:function(resp){
					...
					if(resp.available >= qty){
						...
						if (scheme_id == 1) {
							...
							tr.find('.price').html(resp.price_nhif)
						}
						...
						// alert_toast('kuku'+resp+'')
						// alert_toast("Product quantity is greater than available stock."+resp,'danger')
	...
	$('#manage-sales').submit(function(e){
		...
		$.ajax({
			url:'ajax.php?action=save_sales_insurance',
		    method: 'POST',
		    data: $(this).serialize(),
			success:function(resp){
				if(resp > 0){
					...
					console.log(resp)
					alert_toast("Unsuccessfully",'danger')
				}
			}
		})
	})
