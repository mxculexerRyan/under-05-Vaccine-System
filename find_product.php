		...
		<table class="table table-condensed table-hover" id="plist">
			<colgroup>
				...
				<thead>
					...
					<?php
					if ($_GET["work"]=='pos') {
						?><th class="">Price</th><?php
					}
					?>
					<?php
					if ($_GET["work"]=='invoice') {
						?><th class="">wholesaleprice</th><?php
					}
					?>
					...
					<?php 
						$cat = $conn->query("SELECT * FROM item_categories order by item_category_name asc");
							while($row=$cat->fetch_assoc()):
								$cat_arr[$row['item_category_id']] = $row['item_category_name'];
							endwhile;
						...
						if($available > 0):
					?>
					<tr data-id='<?php echo $row['id'] ?>' data-units="<?=$row['item_units_id']?>">
						<td class=""><b><?php echo $row['name'] ?></b> <sup><?php echo $row['measurement'] ?></sup></td>
						<td class=""><b><?php echo $cat ?></b></td>
						<td class=""><b><?php echo $type_arr[$row['item_units_id']] ?></b></td>
						...
						if($_GET["work"]=='invoice'){
							?>
							<td class="text-right"><b><?php echo number_format($row['wholesale_price'],2) ?></b></td>
							<?php
						}
						?>
...
<script>
	...
	$('#plist tbody tr').click(function(){
		...
		select_units($(this).attr('data-units'))
	})
